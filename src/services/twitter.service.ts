...
export class TwitterService extends BaseService {
  ...
  async postTweet(saleData: SaleData) {
    ...
    const formattedTraits = this.formatTraits(saleData.traits);
  ...
  private formatTraits(traits: any): string {
    // Format traits with each trait on a new line and a bullet point emoji
    return traits.map(trait => `• ${trait.trait_type}: ${trait.value}`).join('\n');
  } 
} 
